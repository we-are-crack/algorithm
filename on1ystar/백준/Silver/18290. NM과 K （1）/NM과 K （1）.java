...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(dfs(matrix, visited, selectedIndex, 0, Integer.MIN_VALUE));
    }
    private static int dfs(String[][] matrix, boolean[][] visited, ArrayList<int[]> selectedIndex, int sum, int max) {
        if(selectedIndex.size() == k) {
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                if(!visited[i][j] && checkNear(selectedIndex, i, j)) {
                    ...
                    max = Math.max(max, dfs(matrix, visited, selectedIndex, sum + Integer.parseInt(matrix[i][j]), max));
                    visited[i][j] = false;
